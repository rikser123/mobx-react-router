...
export const isEqual = <T, K>(elem1: T, elem2: K): boolean => {
    ...
    if (!Array.isArray(elem1) && !Array.isArray(elem2)) {
        ...
        const comparisonByKeys = elem1Keys.map((key) => isEqual(elem1Object[key], elem2Object[key]));
    ...
    const arrayComparison = elem1ArraySorted.map((elem, index) => isEqual(elem, elem2ArraySorted[index]));
...
export const comparePaths = (path: string, routePathTemplate: string) => {
    ...
    const optionalSegments = templateSegments.filter((segment) => segment.startsWith(":") && segment.endsWith("?"));
    const requiredSegments = templateSegments.filter((segment) => !optionalSegments.includes(segment));

    return (
        pathSegments.length >= requiredSegments.length &&
        pathSegments.every((segment, index) => {
            const currentTemplateSegment = requiredSegments[index] || optionalSegments[index - requiredSegments.length];
            const isDynamicTemplate = (currentTemplateSegment ?? "").startsWith(":");

            return currentTemplateSegment && (isDynamicTemplate || segment === currentTemplateSegment);
        })
    );
};
...
export const createFlatPaths = (paths: ConfigRoute[]) => {
    const iter = (paths: ConfigRoute[], acc: FlatPaths, parentSegment: ConfigRoute[]): FlatPaths => {
